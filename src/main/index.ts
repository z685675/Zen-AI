...
import { APP_BUNDLE_ID, APP_EXECUTABLE_NAME } from '@shared/config/constant'
 ...
 */
if (isLinux) {
  app.commandLine.appendSwitch('class', 'ZenAI')
  app.commandLine.appendSwitch('name', APP_EXECUTABLE_NAME)
}
...
if (!app.requestSingleInstanceLock()) {
  ...
  void app.whenReady().then(async () => {
    ...
    // Extract bundled rtk binary to the dedicated app bin directory on first run.
  ...
  app.on('will-quit', async () => {
    // Perform lightweight cleanup without blocking app shutdown.
    if (isOvmsSupported) {
