...
import { APP_BACKUP_PREFIX, APP_PERSIST_KEY } from '@shared/config/constant'
...
export async function backup(skipBackupFile: boolean) {
  const filename = `${APP_BACKUP_PREFIX}.${dayjs().format('YYYYMMDDHHmm')}.zip`
...
export async function backupToWebdav({
  ...
  const backupFileName = customFileName || `${APP_BACKUP_PREFIX}.${timestamp}.${hostname}.${deviceType}.zip`
...
export async function handleData(data: Record<string, any>) {
  if (data.version === 1) {
    ...
    const persistedState = data.localStorage[`persist:${APP_PERSIST_KEY}`] ?? data.localStorage['persist:cherry-studio']
    if (persistedState) {
      localStorage.setItem(`persist:${APP_PERSIST_KEY}`, persistedState)
    }
