...
async function cleanupOldBackups(webdavConfig: WebDavConfig, maxBackups: number): Promise<void> {
  ...
  try {
    ...
    const backupFiles = files
      .filter((file) => file.fileName.startsWith(APP_BACKUP_PREFIX) && file.fileName.endsWith('.zip'))
...
export async function backupToNutstore({
  ...
  const backupFileName = customFileName || `${APP_BACKUP_PREFIX}.${timestamp}.${deviceType}.zip`
