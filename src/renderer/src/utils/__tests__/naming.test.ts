...
describe('naming', () => {
  describe('firstLetter', () => {
    it('should return first letter of string', () => {
      // 验证普通字符串的第一个字�?      expect(firstLetter('Hello')).toBe('H')
    })
  ...
  describe('removeSpecialCharactersForTopicName', () => {
    it('should replace newlines with space for topic name', () => {
      // 验证移除换行符并转换为空�?      expect(removeSpecialCharactersForTopicName('Hello\nWorld')).toBe('Hello World')
    })

    it('should return original string if no newlines', () => {
      // 验证没有换行符的字符�?      expect(removeSpecialCharactersForTopicName('Hello World')).toBe('Hello World')
    })
  ...
  describe('getDefaultGroupName', () => {
    it('should extract group name from ID with slash', () => {
      // 验证从包含斜杠的 ID 中提取组�?      expect(getDefaultGroupName('group/model')).toBe('group')
    })

    it('should extract group name from ID with colon', () => {
      // 验证从包含冒号的 ID 中提取组�?      expect(getDefaultGroupName('group:model')).toBe('group')
    })

    it('should extract group name from ID with space', () => {
      // 验证从包含空格的 ID 中提取组�?      expect(getDefaultGroupName('foo bar')).toBe('foo')
    })

    it('should extract group name from ID with hyphen', () => {
      // 验证从包含连字符�?ID 中提取组�?      expect(getDefaultGroupName('group-subgroup-model')).toBe('group-subgroup')
    })
    ...
    it('should use first and second delimiters for default providers', () => {
      // 默认情况下，'/', ' ', ':' 属于第一类分隔符�?-' '_' 属于第二�?      expect(getDefaultGroupName('Qwen/Qwen3-32B', 'foobar')).toBe('qwen')
  ...
  describe('getBaseModelName', () => {
    ...
    it('should handle edge cases', () => {
      // 验证空字符串的情�?      expect(getBaseModelName('')).toBe('')
      // 验证以分隔符结尾的字符串
      expect(getBaseModelName('model/')).toBe('')
      expect(getBaseModelName('model/name/')).toBe('')
      // 验证以分隔符开头的字符�?      expect(getBaseModelName('/model')).toBe('model')
  ...
  describe('getLowerBaseModelName', () => {
    it('should convert base model name to lowercase', () => {
      // 验证将基础模型名称转换为小�?      expect(getLowerBaseModelName('DeepSeek/DeepSeek-R1')).toBe('deepseek-r1')
  ...
  describe('getFirstCharacter', () => {
    it('should return first character of string', () => {
      // 验证返回字符串的第一个字�?      expect(getFirstCharacter('Hello')).toBe('H')
    })
    ...
    it('should handle special characters and emojis', () => {
      // 验证处理特殊字符和表情符�?      expect(getFirstCharacter('😊Hello')).toBe('😊')
    })
  })
  ...
  describe('getBriefInfo', () => {
    ...
    it('should handle custom max length', () => {
      // 验证自定义最大长�?      const text = 'This is a long text'
  ...
  describe('getFancyProviderName', () => {
    ...
    it('should get name for custom provider', () => {
      ...
      expect(getFancyProviderName(mockProvider)).toBe('好名�?)
    })
  })
