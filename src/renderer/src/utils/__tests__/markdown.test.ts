...
describe('markdown', () => {
  ...
  describe('convertMathFormula', () => {
    ...
    it('should handle multiple delimiters in input', () => {
      // 验证处理输入中的多个分隔�?      const input = 'Text \\[block1\\] and \\(inline\\) and \\[block2\\]'
      const result = convertMathFormula(input)
    ...
    it('should return input if null or empty', () => {
      // 验证空输入或 null 输入时返回原�?      expect(convertMathFormula('')).toBe('')
  ...
  describe('updateCodeBlock', () => {
     ...
     *
     * 使用方法�?     * 1. 修改测试用例，调用该函数
     * 2. 运行测试并查看控制台输出中的代码�?ID
     * 3. 用输出的 ID 替换测试中的硬编�?ID
     * 4. 再次注释掉对此函数的调用
     */
    ...
    it('should only update the second of two identical code blocks', () => {
      // 创建包含两个相同内容代码块的Markdown，文本和代码块交替出�?      const markdown =
