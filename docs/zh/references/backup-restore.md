...
- 仅在真实恢复备份时，清理 `Data/Channels` 中恢复进来的微信登录 token，并将恢复来的微信 channel 标记为离线
...
- 旧微信通道、绑定的会话、权限模式等配置会继续保留。
- 旧电脑的微信登录 token 会在恢复启动阶段删除，旧微信通道会被标记为离线。
- 会话列表里旧微信会话仍可查看，但会显示离线状态，不再代表当前电脑在线接入。
- 用户点击“微信扫码接入”时，如果没有当前可用的在线微信通道，系统会新建一个微信 channel 并弹出新的二维码。
- 新扫码成功的 channel 才代表当前电脑正式在线使用。
- 用户也可以在微信通道设置中对某个通道点击“重新扫码”手动刷新二维码。
