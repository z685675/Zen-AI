...
> 状态：Auto Runtime、内置 Skill、深度研究、个人 Skill 导入和个人定时助手均已实现；当前进行发布回归与持续优化
> 更新日期：2026-08-11
...
此前版本验收中发现的 Office 修复提示、Excel 图表、OCR 和 PPT 风格问题已完成修复。PPT 参考风格现已覆盖参考优先级、图片/图表节奏、明暗比例、构图密度、版式多样性和相似度交付校验；图像密集参考稿不能再以零图片文本稿通过。Codex 浏览器桥接、登录等待、深度研究计时保护、个人 Skill 导入和个人定时助手均已完成实现，后续按功能专项手册执行发布包回归。
...
| 内置 Skill | 十五个生产力与科研 Skill 同时提供给两套 Runtime |
...
| `academic-research` | 文献综述、研究版图、证据卡片和研究缺口分析 |
| `research-design` | 研究问题、假设、实验、指标与可复现性设计 |
| `paper-writing` | 学术论文结构、证据约束写作和投稿前检查 |
| `supervisor-review` | 以导师或审稿人视角给出分级问题与修改建议 |
| `content-writer-cn` | 公众号、小红书、短视频和中文营销写作 |
| `meeting-notes` | 提取决策、未决事项、行动项、负责人和日期 |
| `skill-creator` | 创建、更新和校验结构化 Skill |
...
- 发布前必须通过 Node/Renderer 类型检查、目标测试、i18n 一致性检查和生产构建。
- GitHub Actions 负责生成安装包；Windows 与 macOS 安装包仍需执行启动、升级、托管 Python、模型调用和智能助手主流程实机回归。
- Skill 与定时任务执行 [个人 Skill 导入与定时助手验收手册](./personal-skill-and-scheduler-test.md)。
- 网页读取与登录接管执行 [网页访问与登录接管专项验收手册](./agent-runtime-test-template.md)。
- 长上下文、大文件和缓存需要复验时使用 [统一上下文引擎验收方案](./unified-context-engine-test-plan.md)。

## 11. 当前阶段门禁

| 模块 | 代码 | 自动测试 | 人工验收 |
| --- | --- | --- | --- |
| Auto Runtime 与模型兼容 | 完成 | 通过（含 Grok/Gemini 桥接与 Codex 浏览器桥接） | 发布包回归 |
| 工作区与会话体验 | 完成 | 通过 | 已完成主流程验收 |
| 状态、取消和错误去重 | 完成 | 通过 | 已完成主流程验收 |
| Skill 发现、导入与双目录同步 | 完成 | 通过 | 专项回归 |
| 个人定时助手 | 完成 | 通过 | 专项回归 |
| Markdown/PPTX/DOCX/XLSX/PDF | 完成 | 通过 | 主流程已验收；PPT 设计语言画像、模板密度门禁和自适应设计待候选包回归 |
| 中文写作、会议纪要、研究报告 | 完成 | 脚本校验通过 | 已完成主流程验收 |
| 托管 Python 与 OCR | 完成 | 通过 | 三平台发布包回归 |

相关专项手册中没有阻断问题，当前阶段才算完成。Office 文件要求修复、任务卡死、会话丢失、状态不收口、重复错误卡片、写入错误工作区、Skill 导入后无法启停或定时任务错误丢失，均为阻断问题。
...
### 阶段 C：个人定时助手（已实现，持续优化）

首版实现范围：

- 一次性、间隔和固定时间任务；固定时间是默认选项，界面不要求用户编写 cron 表达式。
- 选择助手、工作区、执行模型和任务提示词；普通模式只展示产品允许的三款模型，开发者模式可查看全部可用模型。
- 执行模型只决定实际请求模型，底层仍由 Auto Runtime 按模型与协议选择 Codex 或 Claude Code。
- 创建和编辑时校验执行时间与调度值，一次性任务不能保存过去的时间。
- 应用启动时恢复活动任务；应用关闭或电脑关机期间不执行，重新打开后按下一次计划继续调度，不伪造已执行结果。
- 支持手动立即执行、再次执行、暂停、恢复、删除、最近结果、失败原因和运行日志；每次执行创建独立会话，日志可直接跳转并关闭任务窗口。
- 只有会话已经创建、任务进入明确终态且结果可读取时才记录成功；需要用户确认时进入等待状态，并在应用任意页面弹出操作提醒。
- 不设置固定总时长上限；连续 15 分钟没有模型输出、工具调用或状态变化时按卡住处理并终止本次运行。
...
- 2026-08-05：补充学术研究、研究设计、论文写作和导师审阅 Skill，科研任务按“研究流程、证据获取、写作交付、质量审阅”分层编排。
- 2026-08-11：定时任务改为每次执行创建独立会话；成功状态以会话和任务终态共同收口，并增加卡住检测、全局用户操作提醒和可跳转运行历史。
