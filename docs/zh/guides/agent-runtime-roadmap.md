...
> 状态：阶段 A 功能开发完成；PPT 设计语言画像、原生模板预检、自适应设计分流与硬性交付闸门已实现，待用户实机复验、签名安装包和发布工程回归
> 更新日期：2026-07-26
...
3. Claude Code 与 Codex 共用会话、MCP、Skill、文件生成、Python 和 OCR 能力。
4. Office/PDF 文件优先由内置 Node 生成器创建，托管 Python 负责数据处理和质量校验。
5. 本阶段只面向个人本机使用；团队协作不进入当前实现和验收。

当前统一验收已完整执行，期间发现的 Office 修复提示、Excel 图表、OCR 和 PPT 风格问题已完成修复。PPT 参考风格现已覆盖参考优先级、图片/图表节奏、明暗比例、构图密度、版式多样性和相似度交付校验；图像密集参考稿不能再以零图片文本稿通过。`v1.1.58` Windows x64 unpacked 候选目录已生成，Claude/Codex、Skill、字体、托管 Python 安装脚本和系统 OCR 资源检查通过。进入正式发布前仍需完成本轮 PPT 实机复验、签名安装包及发布工程回归；完成前不继续扩展 Skill 导入或定时助手。
...
- 用户 Skill 导入、预览、冲突处理、启停和升级。
- 面向个人的定时助手入口、任务管理和通知体验。
- 团队工作区、权限共享和多人任务协作。
...
| 模块 | 代码 | 自动测试 | 人工验收 |
| --- | --- | --- | --- |
| Auto Runtime 与模型兼容 | 完成 | 通过（含 Grok/Gemini 桥接与 ToolSearch 回传） | GPT、Claude、Gemini、Grok 已跑通 |
| 工作区与会话体验 | 完成 | 通过 | 已完成主流程验收 |
| 状态、取消和错误去重 | 完成 | 通过 | 已完成主流程验收 |
| Skill 发现与双目录同步 | 完成 | 通过 | 已完成主流程验收 |
...
只有统一手册中没有阻断问题，当前阶段才算完成。Office 文件要求修复、任务卡死、会话丢失、状态不收口、重复错误卡片或写入错误工作区，均为阻断问题。

## 12. 后续开发路线

### 阶段 A：当前能力验收与发布收口

1. 已从空工作区执行 T01-T26 与 T04-GX，多协议模型和自动视觉风格已完成实机验收。
2. 已修复验收发现的阻断问题，并完成相关生成链路和 PPT 风格回归。
3. `v1.1.58` Windows x64 unpacked 候选目录及 Claude/Codex、Skill、字体、托管 Python 安装脚本和系统 OCR 资源已检查通过；继续在候选版本执行 T11-R/T11-N。
4. 收口 Windows Vitest 零退出码和硬编码文本增量基线，确保发布门禁可以稳定重复执行。
5. 提交或创建可回滚快照，生成并验证签名安装包后发布候选版本；候选包回归通过后转正式版本。

### 阶段 B：个人 Skill 导入

验收通过后优先开发 Skill 导入，因为它直接提升复杂创作和连续任务的成功率。

首版范围：

- 从本地文件夹或 ZIP 导入结构化 Skill。
- 导入前展示名称、说明、脚本、资源和权限风险。
- 校验 `SKILL.md`、路径、脚本入口和依赖声明。
- 支持启用、停用、删除、版本冲突处理和一键回滚。
- 同一份 Skill 自动同步到 `.claude/skills` 与 `.agents/skills`。
- 不在首版开放任意安装系统软件或无确认执行未知脚本。

### 阶段 C：个人定时助手

Skill 导入稳定后再开发个人定时助手：

- 一次性、周期性和固定时间任务。
- 选择助手、模型、工作区和结果通知方式。
- 本机离线/关机时明确显示错过或待补跑状态。
- 任务历史、最近结果、失败原因、暂停和删除。
- 复用 Auto Runtime、Skill、权限和单一终态机制。
