#!/usr/bin/env python3
"""Load a built-in PPTX deck template and prepare an editable deck spec."""
...
TEMPLATE_DIR = Path(__file__).resolve().parents[1] / "assets" / "deck-templates"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare a deck spec from a built-in PPTX template.")
    parser.add_argument("template", nargs="?", help="Template name, e.g. executive-report.")
    parser.add_argument("--title", help="Override the deck title.")
    parser.add_argument("--out", help="Write the prepared deck spec to this path.")
    ...
    if args.title:
        ...
        if spec.get("slides"):
            spec["slides"][0]["title"] = args.title
