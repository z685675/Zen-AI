...
function check(content: string): void {
  if (!fs.existsSync(OUTPUT_FILE)) {
    console.error(`openapi:check failed 鈥?${OUTPUT_FILE} does not exist (run pnpm generate:openapi)`)
    process.exit(1)
  }
  ...
  if (JSON.stringify(existing) !== JSON.stringify(generated)) {
    console.error('openapi:check failed 鈥?openapi-spec.json is out of date (run pnpm generate:openapi)')
    process.exit(1)
  }
